...
export const QUALITY_POLICY_VERSION = "GB-ACE-DELIVERY-V12-TEST-DISCLAIMER-GUARD";
...
export function findUnresolvedPlaceholders(body: string): string[] {
  ...
  if (UNRESOLVED_OPTION_PATTERN.test(body)) reasons.push("未闭环的变量或方案选择");
  return reasons;
}
...
export function findUnsupportedClaims(body: string): string[] {
  ...
  for (const sentence of sentences) {
    const negatedOrPlanned = /(?:未执行|未完成|未验证|未测试|未实测|未输出|尚未|不得|不能|不应|不构成|不代表|不属于|不视为|不预设|不输出|不出具|并非|禁止|计划|规划|假设|示例|建议|假装)/i.test(sentence);
