...
- V8 same-task rework on Worker `acc1ae67-c4d7-4262-986f-35c3dcd05de3` revalidated G00 (first candidate rejected for evidence/claim issues; corrected candidate accepted) and then blocked G01 after three attempts. The final candidate omitted the server-required uploaded STEP ID, so the mandatory input traceability gate rejected it. A new local prompt constraint now injects the exact server-verified ID list into the body/evidence instructions and emits targeted retry feedback; production retest is pending.
- Review of the terminal V8 G01 candidate showed the completion detector also blocked the explicit sentence “本阶段不输出已完成的测试结论.” V9 locally recognizes `不输出/不出具` as a disclaimer and additionally rejects unsourced quantified electrical/utility specifications; regressions cover both cases. Full V9 tests are 62/62 PASS with TypeScript/Wrangler type checks and diff check clean; V9 deploy/rework pending.
- V9 final G01 candidate review traced the CAD-unit block to `G02` being parsed as a numeric dimension in a “defer to G02 for unit confirmation” sentence. This is a false positive. V10 locally removes stage-ID tokens only before the unit/dimension regexes; exact-number unit checks are unchanged. Production retest pending.
- V10 production rework no longer reported a G02-as-dimension rejection. It still blocked G01 on all three attempts; final rejected text explicitly said “本阶段不预设任何已完成测试结论,” so this is a completion-detector false positive. V11 adds `不预设` as a disclaimer with a negative fixture; the existing positive FAT/test rejection remains. Production retest pending.
- V11 same-task rework accepted G00/G01/G03. G04 was `QUALITY_BLOCKED` after three attempts; the final candidate explicitly stated “未输出任何 CAD 派生长度或已完成测试结论,” but the detector did not recognize `未输出`. V12 adds that disclaimer plus a regression; this is another false-positive fix, pending verification.
- V12 rework stopped at G00. Final candidate said “未声明任何已完成测试,” which still triggered the completion claim detector. V13 adds explicit `未声明/未声称/未宣称` disavowal tokens with a regression; production validation pending.
- V14 disposition (2026-09-24): latest same-task run accepted G00/G01/G03/G04/G05/G06/G07/G02/G08, then ended `QUALITY_BLOCKED` at G09 after three attempts. Inspection also exposed named camera/lens suppliers and models, component part numbers, and fabricated 2026-Q1 market-price claims without any quotation source. No ZIP was produced or approved. V14 adds a generic server-side BOM sourcing gate and regression cases; focused quality/Harness tests are 20/20 PASS and TypeScript compile is PASS. Production deployment/retest is still pending. Until a V14 live retest and full artifact review: no quote-backed BOM, ZIP download claim, quality PASS, or GPT-SOL/golden parity claim.
- V14 same-task live rework instead stopped in G00. Review of the exact rejected candidate showed the G00 text was not claiming completed FAT/field tests: `RULE-...V14` was parsed as a default metric and the phrases `待验证` / `CADCore 已验证拓扑计数` were mistaken for completed tests. These are documented false positives, not grounds to waive the strict gates. V15 has targeted fixes and exact-candidate regressions; production rework remains pending.
- V15 local verification: 13/13 focused quality tests, 64/64 full tests, TypeScript compile, Wrangler types, deploy dry-run, and diff check all PASS. Worker publication and same-task production replay have not yet happened. Do not treat the blocked V14 G00 result as a production pass.
- V15 was deployed and same-task rework accepted G00; G01 blocked after three candidates tripped the CAD-unit gate. Review of the final rejected body showed the match was the provider provenance string `MiniMax-M3`; the body also contained a truncated JSON transport response and was not stage-ready. V16 has a narrow metadata-aware detector regression (`MiniMax-M3` ignored only in provider metadata; actual `M3` thread claim still caught). Deploy and same-task rework pending.
- V16 local verification: focused quality 13/13 PASS, full suite 64/64 PASS, TypeScript/Wrangler types/deploy dry-run/diff check PASS. Production deployment and same-task replay not yet performed.
- V16 was deployed and the same task stopped at G01 after three quality candidates were rejected. The final candidate was truncated schema-shaped JSON containing engineering-looking text. It was not accepted; V17 moves this malformed-envelope failure ahead of candidate persistence while retaining genuine prose fallback. V17 checks and replay pending.
- V17 local verification: 22/22 focused provider/quality tests, 65/65 full tests, TypeScript, Wrangler types, deploy dry-run, and diff check all PASS. Production deployment and same-task replay pending.
- V17 same-task production disposition: G00 and G01 passed; G03 `QUALITY_BLOCKED` after three attempts. The final body asserted STEP units were mm despite server `UNCONFIRMED`, and retained interface/light/protocol alternatives (“任一均可”, “中选取”). V18 addresses exact observed gaps while retaining the hard unit guard. Focused tests are 20/20 PASS; full checks and production replay pending.
- V18 local verification: 20/20 focused quality/Harness tests, 65/65 full tests, TypeScript, Wrangler types, deploy dry-run, and diff check all PASS. Worker deployment and same-task replay pending.
- V18 production same-task rework (2026-09-24): G00–G04 passed and G05 blocked after three attempts. Two G05 candidates were rejected for unsourced quantified engineering specifications and numeric dimensions while CADCore unit status was `UNCONFIRMED`; no G05 artifact or ZIP was released. Manual review also found unresolved interface alternatives and deferred confirmations in accepted G03/G04.
- V19 root cause/fix: `evaluateCandidate()` checked only the literal placeholder regex and did not call the existing `findUnresolvedPlaceholders()` helper. V19 wires that helper into candidate acceptance and expands coverage for deferred confirmations and ambiguous alternatives. Focused quality/Harness tests: 21/21 PASS; full suite: 66/66 PASS; TypeScript/Wrangler types/deploy dry-run/diff check: PASS. Production deploy and same-task rework are pending.
- V19 same-task production replay stopped at G00 after 3/3 candidate rejections for unresolved placeholder/open-item content. This was correct for the stale `UNCONFIRMED` report but prompted a source check before asking the user for data.
...
- V21 reuses the canonical job row instead of inserting a duplicate during legacy STEP report regeneration. V21 verification and same-task replay pending.
- V21 local verification: Python parser unit tests 3/3, full suite 66/66, TypeScript/Wrangler types/deploy dry-run/diff check PASS. Re-deployment and production replay pending.
